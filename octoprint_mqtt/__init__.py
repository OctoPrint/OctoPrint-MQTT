...
class MqttPlugin(
    ...
    EVENT_CLASS_TO_EVENT_LIST = dict(
        ...
        slicing=(
            ...
            Events.SLICING_PROFILE_ADDED,
            Events.SLICING_PROFILE_DELETED,
    ...
    def get_template_configs(self):
        return [dict(type="settings", name="MQTT")]
    ...
    def on_settings_save(self, data):
        ...
        if len(broker_diff) or len(lw_diff):
            # something changed
            self._logger.info(
                "Settings changed (broker_diff={!r}, lw_diff={!r}), reconnecting to broker".format(
                    broker_diff, lw_diff
                )
            )
        ...
        if topic:
            for key, value in data.items():
                ...
                if (
                    ...
                    dataset = dict(actual=value["actual"], target=value["target"])
                    self.mqtt_publish_with_timestamp(
    ...
    def mqtt_publish(self, topic, payload, retained=False, qos=0, allow_queueing=False):
        ...
        if not self._mqtt_connected:
            if allow_queueing:
                self._logger.debug(
                    "Not connected, enqueuing message: {topic} - {payload}".format(
                        **locals()
                    )
                )
    ...
    def mqtt_unsubscribe(self, callback, topic=None):
        subbed_topics = [
            ...
            if callback == subbed_callback and (topic is None or topic == subbed_topic)
        ]

        def remove_sub(entry):
            subbed_topic, subbed_callback, _, _ = entry
            return not (
                callback == subbed_callback and (topic is None or subbed_topic == topic)
            )
    ...
    def _on_mqtt_connect(self, client, userdata, flags, rc):
        ...
        if not rc == 0:
            ...
            self._logger.error(
                reason if reason else "Connection to mqtt broker refused, unknown error"
            )
            return
        ...
        subbed_topics = list(
            map(
                lambda t: (t, 0), {topic for topic, _, _, _ in self._mqtt_subscriptions}
            )
        )
        if subbed_topics:
    ...
    def _on_mqtt_disconnect(self, client, userdata, rc):
        ...
        if not rc == 0:
            self._logger.error(
                "Disconnected from mqtt broker for unknown reasons (network error?), rc = {}".format(
                    rc
                )
            )
        else:
